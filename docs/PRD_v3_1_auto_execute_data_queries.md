...
**Critical APIs:**
- `handleGeneration({ question, userIdentifier, model })` - Generate SQL (returns `{ ok, sql, query_type, explanation, metadata }`)
- `validateSQL(sql, { schemaSnapshotId, queryType })` - Returns `{ valid, sqlWithLimit, violations }`
- `setJobResult(jobId, result)` - Mark job completed
- `setJobError(jobId, errorMessage)` - Mark job failed
- `updateJob(jobId, JobStatus.PROCESSING)` - Update job status

**Required Data Query Columns:**
- `date` (TEXT/TIMESTAMP) - Test date
- `value` (NUMERIC) - Measurement value
- `unit` (TEXT) - Unit of measurement
- `reference_interval` (TEXT) - Formatted interval (e.g., '< 5.2', '30 - 100')

**Security Rules:**
1. Execute `validation.sqlWithLimit` only (never raw SQL)
2. Strip SQL from data query responses before returning to frontend: `const { sql, ...resultWithoutSql } = ...`
3. Use 4-layer validation (regex + EXPLAIN + LIMIT + timeout)
4. Set timeouts via `SET LOCAL statement_timeout` inside transactions (auto-reverts)

**Frontend Flow:**
1. Poll job endpoint → `GET /api/sql-generator/jobs/:jobId`
2. Check `job.result.query_type` (default to 'data_query' if missing)
3. Route: `plot_query` → render chart, `data_query` → render table
4. Refactor `renderSqlResult()`, don't delete it (dispatcher function)

**Response Structure:**
```javascript
// Job envelope (all query types)
{ jobId, status, result: { /* generation result */ }, error }

// Data query result (inside job.result)
{ ok: true, query_type: 'data_query', execution: { rows, rowCount, totalRowCount }, metadata }
// NOTE: No 'sql' field in data query results

// Plot query result (inside job.result) - UNCHANGED
{ ok: true, sql: '...', query_type: 'plot_query', plot_metadata, metadata }
```
...
1. **DO NOT DELETE `renderSqlResult()` function** (lines ~1251-1358 in `public/js/app.js`)
   - It's the main dispatcher called by job polling logic (line ~1458)
   - Deleting it breaks ALL SQL generation (plots AND data queries)
   - **REFACTOR it, don't delete it**
   - Keep: Chart cleanup, UI state management
   - Remove: SQL text box population code
   - Add: Data query routing branch

2. **ALWAYS use `validation.sqlWithLimit` for execution** (SECURITY CRITICAL)
   - Never execute raw LLM-generated SQL
   - The validator MODIFIES SQL to inject/clamp LIMIT
   - Using `validation.sql` or raw SQL is a **security vulnerability**
   - Pattern: `const validation = await validateSQL(rawSql, {...}); await executeQuery(validation.sqlWithLimit);`

3. **Strip SQL from data query responses**
   - Use destructuring: `const { sql, ...resultWithoutSql } = sqlGenerationResult;`
   - Ensures SQL never reaches browser for data queries
   - Plot queries still need SQL (existing behavior)

4. **Use correct Job Manager APIs:**
   - `updateJob(jobId, JobStatus.PROCESSING)` - Update status
   - `setJobResult(jobId, result)` - Mark completed (NOT `updateJob(jobId, 'completed', ...)`)
   - `setJobError(jobId, errorMessage)` - Mark failed

5. **Use correct database pool pattern:**
   - Import: `const { pool } = require('../db');`
   - NOT: `dbPool.getPool()` or other patterns

6. **Handle missing `query_type` gracefully:**
   - Single-shot mode doesn't return `query_type` field
   - Always default: `const queryType = result.query_type || 'data_query';`
   - Log warning when field is missing

---

### Response Structure & Job Envelope

**IMPORTANT:** The job endpoint returns a job envelope containing the SQL generation result. Understanding this structure is critical before implementing backend changes.

The frontend polls `GET /api/sql-generator/jobs/:jobId` and receives:
...
setImmediate(async () => {
  try {
    ...
    // Log warning if query_type is missing (helps debug when AGENTIC_SQL_ENABLED=false)
    if (!sqlGenerationResult?.query_type) {
      console.warn(
        `[sqlGenerator:${requestId}] query_type field missing (agentic mode disabled or error?). ` +
        `Defaulting to 'data_query' with auto-execution. ` +
        `Check AGENTIC_SQL_ENABLED env var. SQL will be validated before execution.`,
        {
          jobId,
          hasAgenticMetadata: !!sqlGenerationResult?.metadata?.agentic
        }
      );
    }
    ...
    // Double validation is intentional defense-in-depth:
    // - First validation in handleGeneration() occurs during SQL generation
    // - Second validation here ensures no bugs in generation service
    // - Also revalidates with explicit queryType parameter for query_type-specific constraints
    // - The validator returns modified SQL in validation.sqlWithLimit (LIMIT injected/clamped)
    const validation = await validateSQL(rawSql, {
      schemaSnapshotId: sqlGenerationResult.metadata?.schema_snapshot_id,
      queryType: 'data_query'  // Explicit query type for LIMIT enforcement (50 rows for data queries)
    });
    ...
    // Validate required column aliases (fail fast before returning to frontend)
    if (executionResult.rowCount > 0) {
      const firstRow = executionResult.rows[0];
      const requiredColumns = ['date', 'value', 'unit', 'reference_interval'];
      const missingColumns = requiredColumns.filter(col => !(col in firstRow));

      if (missingColumns.length > 0) {
        const errorMsg = `Query missing required columns: ${missingColumns.join(', ')}. ` +
          `The LLM must generate SQL with these aliases for data queries.`;
        console.error(`[sqlGenerator:${requestId}] ${errorMsg}`, {
          sql_preview: rawSql.substring(0, 200),
          missingColumns
        });
        setJobError(jobId, 'Query results are incomplete. Please try rephrasing your question.');
        return;
      }
    }
...
**Test Matrix for COUNT Query Edge Cases:**

Engineers should verify `countTotalRows()` handles these scenarios correctly:

```javascript
// Test cases for LIMIT stripping logic
const testCases = [
  {
    input: 'SELECT * FROM lab_results LIMIT 50',
    expected: 'SELECT * FROM lab_results',
    description: 'Simple LIMIT clause'
  },
  {
    input: 'SELECT * FROM lab_results LIMIT 50 OFFSET 10',
    expected: 'SELECT * FROM lab_results',
    description: 'LIMIT with OFFSET (both removed)'
  },
  {
    input: 'SELECT * FROM lab_results LIMIT 50;',
    expected: 'SELECT * FROM lab_results',
    description: 'LIMIT with trailing semicolon'
  },
  {
    input: 'SELECT * FROM lab_results LIMIT 50 -- pagination',
    expected: 'SELECT * FROM lab_results',
    description: 'LIMIT with trailing comment'
  },
  {
    input: 'SELECT * FROM lab_results LIMIT 50 OFFSET 10;',
    expected: 'SELECT * FROM lab_results',
    description: 'LIMIT with OFFSET and semicolon'
  },
  {
    input: 'SELECT * FROM lab_results LIMIT 50 OFFSET 10 -- page 2\n',
    expected: 'SELECT * FROM lab_results',
    description: 'LIMIT with OFFSET, comment, and newline'
  }
];

// Expected behavior:
// - Trailing artifacts (;, comments, whitespace) stripped first
// - Then LIMIT + optional OFFSET removed
// - Result wrapped in COUNT query
// - Returns total rows (not capped at LIMIT value)
```

**Unit Test Recommendation:**
Add these test cases to `test/unit/countTotalRows.test.js` to prevent regressions.
 ...
 * @returns {string}
 *
 * NOTE: Date format uses 'en-US' locale for consistency across all users
 * Rationale for fixed locale:
 * - Consistent format across Russian and English-speaking users
 * - "Jan 15, 2024" is universally understood
 * - Avoids confusion from different regional date formats (MM/DD vs DD/MM)
 *
 * Future Enhancement (out of scope for MVP):
 * - Add user preference for date format (Settings page)
 * - Or: Auto-detect locale: navigator.language (e.g., 'ru-RU' for Russian users)
 * - Implementation: d.toLocaleDateString(navigator.language || 'en-US', {...})
 */
function formatDate(date) {
  ...
  try {
    ...
    // Date format: Always 'en-US' for consistency across locales
    // TODO: Add user preference in future release (see function doc above)
...
@media (max-width: 768px) {
  ...
  .row-count-message {
    font-size: 0.85rem;  /* Adjust message size for mobile */
  }
}
```
...
- Table remains full-width with touch-friendly scrolling
- Text size slightly reduced on mobile for better fit
- No custom mobile layout (out of scope for MVP)
...
**If v3.1 causes issues and needs to be reverted:**

1. **Revert backend changes:**
   - Deploy previous version of `server/routes/sqlGenerator.js` (no auto-execution for data queries)
   - Remove `executeDataQuery()` and `countTotalRows()` helper functions
   - Data queries will return SQL only (original behavior)

2. **Revert frontend changes:**
   - Restore SQL text box (`#sql-output` div)
   - Restore "Copy SQL" button
   - Restore SQL explanation box (`#sql-notes`)
   - Remove data results table section
   - Restore original `renderSqlResult()` function (displays SQL, not results)
   - Restore related CSS styles

3. **No data loss:**
   - Job logs contain all executed SQL queries
   - No database schema changes (no migration rollback needed)
   - No persistent state to clean up

4. **Time estimate:**
   - 30 minutes to roll back code changes
   - 15 minutes to redeploy and verify
   - Total: ~45 minutes

**Rollback triggers:**
- Execution failure rate >10%
- User complaints about missing SQL visibility
- Performance degradation (queries >5s p95)
- Security concerns with auto-execution
- Critical bugs in column alias validation

**Rollback procedure:**
```bash
# 1. Revert to previous commit
git revert <commit-hash-of-v3.1>

# 2. Deploy to staging
npm run deploy:staging

# 3. Verify rollback
curl -X POST https://staging.healthup.app/api/sql-generator \
  -H "Content-Type: application/json" \
  -d '{"question": "show my latest cholesterol"}'
# Expect: SQL returned, not execution results

# 4. Deploy to production
npm run deploy:production

# 5. Monitor logs
tail -f /var/log/healthup/app.log | grep sqlGenerator
```
...
All user-facing data is rendered using XSS-safe patterns:

1. **Table Cell Rendering:**
   - Uses `textContent`, NOT `innerHTML` for all cell values
   - Example: `dateCell.textContent = formatDate(row.date);`
   - This automatically escapes HTML special characters (<, >, &, etc.)

2. **Error Messages:**
   - Passed through `escapeHtml()` function before display
   - Creates DOM element with `textContent`, then reads `innerHTML`
   - Pattern: `tbody.innerHTML = '<tr><td>' + escapeHtml(message) + '</td></tr>';`

3. **Dynamic Attributes:**
   - Only safe attributes used (`data-out-of-range`, `colspan`)
   - No `onclick`, `onload`, or other event handlers in dynamic content

4. **User Input Sanitization:**
   - SQL queries never exposed to frontend for data queries
   - OCR-extracted data sanitized before storage (existing validation)
   - LLM-generated content (explanations) already sanitized in backend

**Backend SQL Injection Prevention:**

4-layer validation prevents SQL injection:
1. **Regex validation:** Forbids INSERT, UPDATE, DELETE, DROP, etc.
2. **EXPLAIN plan:** Proves query is read-only before execution
3. **LIMIT enforcement:** Validator injects/clamps LIMIT clause
4. **Timeout enforcement:** 30-second max execution, 5-second max for COUNT

**Attack Scenarios Mitigated:**

| Attack Vector | Mitigation |
|---------------|------------|
| XSS via data values (e.g., `<script>alert(1)</script>` in lab result) | `textContent` rendering auto-escapes |
| XSS via error messages | `escapeHtml()` function sanitizes before display |
| SQL injection via user question | 4-layer validation + read-only enforcement |
| SQL injection via LLM-generated SQL | EXPLAIN plan proves read-only, LIMIT enforced |
| Command injection via SQL | Parameterized queries (not string concat), pool.query() safe |

**Testing Recommendations:**
- Inject `<script>alert('XSS')</script>` in lab report PDFs
- Verify frontend renders as plain text (not executed)
- Test error messages with HTML entities
- Attempt SQL injection via question field
- Verify EXPLAIN plan blocks non-SELECT statements
